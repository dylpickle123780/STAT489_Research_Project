...
# Introduction {#sec-intro}
  ...
  The United States is infamous for its housing unaffordability crisis with over half of American Renters unable to afford rent according to the Joint Center for Housing Studies at Harvard University [@harvard_americas_2023]. However, with the government allocating 72.6 Billion dollars to be spent in 2025 for the Department of Housing and Urban Development with an additional 185 billion dollars to be spent to solve the housing crisis over the next ten years, there has to be a way to allocate the funds to maximize people being able to afford their rent [@HUD_fair_2023]. 
...
```
  ...
  We used the Department of Housing’s Fair market rent data from 2019 for our fair market data and also log transformed the pricing [@HUD_fair_2023]. They collected this data by finding the 40th percentile income of any given county in the United States and saying that 30% of their income is a "fair market rent" hence the name. There were roughly 4,400 observations accounting for rent areas across the United States at roughly the county level which provided for the baseline figures we used for the comparison. 
...
ggarrange(simpl_1,simpl_2,simpl_3,simpl_4, common.legend = TRUE, labels = c("Sq_ft","Pop","Bed","Bath"))
...
\end{figure}
  ...
  Using the real data and predicting at every county we were able to find a predictive surface that showed rent pricing on a log scale as a function of space \cref{fig:pred_surface}. For the prediction we used the median of bathrooms and bedrooms and the mean of population and square footage to hold all covariates constant. 
...
```{r nonstationarity, fig.align='center'}
...
```{r, message=FALSE, results = "hide", cache=TRUE, warning=FALSE, output = FALSE}
library(ggplot2)
ggarrange(pl_1,pl_2,pl_3,pl_4, common.legend = TRUE, labels = c("Bed","Bath","Sq_ft","Pop"))
ggsave("Coefficients_real_data.png", path = "./report_images/figure-pdf")
```
\begin{figure}[ht]
\centering
\includegraphics{./report_images/figure-pdf/Coefficients_real_data.png}
\caption{}
\label{fig:real_coef}
\end{figure}
