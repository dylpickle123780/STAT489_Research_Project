...
\begin{figure}[h]
  \centering
  \includegraphics[width = 0.9\textwidth]{./report_images/figure-pdf/Sim_coefficients.png}
...
When creating a GWR model the two most important hyperparameters are the kernel and bandwidth. We decided to use an exponential kernel due to how sharp some of the changes are between counties along the coastline and around the Denver, Colorado area. We estimated the bandwidth using cross validation and came upon `r round(model_bandwidth,2)` as the best fit.
